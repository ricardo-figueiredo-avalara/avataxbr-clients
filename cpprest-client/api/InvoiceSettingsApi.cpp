...
InvoiceSettingsApi::InvoiceSettingsApi( std::shared_ptr<ApiClient> apiClient )
...
pplx::task<std::shared_ptr<Inline_response_200_4>> InvoiceSettingsApi::invoiceSettings()
{
    

    std::shared_ptr<ApiConfiguration> apiConfiguration( m_ApiClient->getConfiguration() );
    utility::string_t path = U("/invoices/settings");
    ...
    {
        throw ApiException(400, U("InvoiceSettingsApi->invoiceSettings does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("InvoiceSettingsApi->invoiceSettings does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling invoiceSettings: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling invoiceSettings: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
