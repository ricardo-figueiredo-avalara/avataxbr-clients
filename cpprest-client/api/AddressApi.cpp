...
pplx::task<std::vector<std::shared_ptr<Address>>> AddressApi::getZipCode(utility::string_t zipcode)
{
    

    std::shared_ptr<ApiConfiguration> apiConfiguration( m_ApiClient->getConfiguration() );
    utility::string_t path = U("/addresses/{zipcode}");
    boost::replace_all(path, U("{") U("zipcode") U("}"), ApiClient::parameterToString(zipcode));
    ...
    {
        throw ApiException(400, U("AddressApi->getZipCode does not produce any supported media type"));
    }
    ...
    {
        throw ApiException(415, U("AddressApi->getZipCode does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling getZipCode: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling getZipCode: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
    ...
    .then([=](utility::string_t response)
    {
        ...
        if(responseHttpContentType == U("application/json"))
        {
            ...
            {
                std::shared_ptr<Address> itemObj(new Address());
