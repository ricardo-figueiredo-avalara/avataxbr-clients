...
InvoiceNFCeDANFEApi::InvoiceNFCeDANFEApi( std::shared_ptr<ApiClient> apiClient )
...
pplx::task<HttpContent> InvoiceNFCeDANFEApi::invoicesNfceKeyGet(utility::string_t authorization, utility::string_t key)
{
    

    std::shared_ptr<ApiConfiguration> apiConfiguration( m_ApiClient->getConfiguration() );
    utility::string_t path = U("/invoices/nfce/{key}");
    ...
    {
        throw ApiException(415, U("InvoiceNFCeDANFEApi->invoicesNfceKeyGet does not consume any supported media type"));
    }
    ...
    .then([=](web::http::http_response response)
    {
        ...
        if (response.status_code() >= 400)
        {
            throw ApiException(response.status_code()
                , U("error calling invoicesNfceKeyGet: ") + response.reason_phrase()
        ...
        if(response.headers().has(U("Content-Type")))
        {
            ...
            if( contentType.find(responseHttpContentType) == std::string::npos )
            {
                throw ApiException(500
                    , U("error calling invoicesNfceKeyGet: unexpected response type: ") + contentType
                    , std::make_shared<std::stringstream>(response.extract_utf8string(true).get()));
            }
        }
