...
entityCode: Option[String],  // This string is a code maintained by the client application and recorded in CUP to uniquely identify the party with whom the company (identified by companyCode) is transacting. It is unique within the context of a Company.
...
transactionDate: Option[String],  // This string is the transaction date in ISO 8601 format, create transaction date
...
invoiceNumber: Option[Integer],  // Invoice number, sequential unique by invoice serial (Número da nota fiscal)
