...
+ (instancetype)sharedAPI {
    static SWGAddressApi *sharedAPI;
...
-(NSNumber*) getZipCodeWithZipcode: (NSString*) zipcode
    ...
    if (zipcode == nil) {
        NSParameterAssert(zipcode);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"zipcode"] };
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/addresses/{zipcode}"];
