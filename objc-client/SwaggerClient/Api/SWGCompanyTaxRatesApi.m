#import "SWGCompanyTaxRatesApi.h"
...
@interface SWGCompanyTaxRatesApi ()

@property (nonatomic, strong) NSMutableDictionary *defaultHeaders;

@end

@implementation SWGCompanyTaxRatesApi

NSString* kSWGCompanyTaxRatesApiErrorDomain = @"SWGCompanyTaxRatesApiErrorDomain";
NSInteger kSWGCompanyTaxRatesApiMissingParamErrorCode = 234513;
...
+ (instancetype)sharedAPI {
    static SWGCompanyTaxRatesApi *sharedAPI;
...
-(NSNumber*) companiesCompanyIdTaxratesGetWithCompanyId: (NSString*) companyId
    ...
    if (companyId == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kSWGCompanyTaxRatesApiErrorDomain code:kSWGCompanyTaxRatesApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/companies/{companyId}/taxrates"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"NSArray<SWGCustomTaxTypeRate>*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((NSArray<SWGCustomTaxTypeRate>*)data, error);
                                }
                           }
          ];
}
...
-(NSNumber*) companiesCompanyIdTaxratesTaxTypeGetWithCompanyId: (NSString*) companyId
    ...
    if (taxType == nil) {
        NSParameterAssert(taxType);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"taxType"] };
            NSError* error = [NSError errorWithDomain:kSWGCompanyTaxRatesApiErrorDomain code:kSWGCompanyTaxRatesApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/companies/{companyId}/taxrates/{taxType}"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"SWGCustomTaxTypeRate*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((SWGCustomTaxTypeRate*)data, error);
                                }
                           }
          ];
}
